...
- **Watched mods** — past a couple of hundred tracked mods, "every update" is noise. Ring the bell on a mod (library row, detail sheet, or the *Watched* filter chip) and only those announce updates. Nothing is watched by default, and an unwatched mod's update raises no event at all, so it reaches neither the feed here nor any downstream consumer. Removals, bans and your own completed downloads are always reported, watched or not.
  The line is **watching vs tracking**, and only announcements move: an unwatched mod is still polled, its `lastSeenRemoteTs` still advances, and its state still resolves to *Awaiting Steam* / *Queued for launch*. So it keeps appearing in the library, in the Launch Queue and in the pending badge — those are a standing answer to "what is out of date?", not an interruption. Only the event stream is filtered.
